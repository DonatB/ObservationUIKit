...
extension UIViewController {
        ...
        observe({ [weak self] in
            ...
            if let unwrappedItem = item.wrappedValue {
                ...
                func presentNewController() {
                    ...
                    presented = Presented(
                        controller: controller, id: unwrappedItem.id
                    )
                    present(controller, animated: true)
                }
                
                if let presented {
                    guard AnyHashable(unwrappedItem.id) != presented.id else { return }
    ...
    fileprivate var presented: [AnyHashable: Presented] {
        get {
            objc_getAssociatedObject(self, presentedKey) as? [AnyHashable: Presented] ?? [:]
        }
        set {
    ...
    fileprivate var onDeinit: OnDeinit? {
        get {
            objc_getAssociatedObject(self, onDeinitKey)
            as? OnDeinit
        }
        set {
            objc_setAssociatedObject(
...
extension UINavigationController {
    func pushViewController<Item>(item: @autoclosure @escaping () -> Binding<Item?>, content: @escaping (Item) -> UIViewController) {
        observe({ [weak self] in
            guard let self else { return }
            let item = item()
    ...
    private func popFromViewController(
      ...
      popToViewController(
        viewControllers[index - 1], animated: true
      )
    }
...
@MainActor private let presentedKey = malloc(1)!
@MainActor private let onDeinitKey = malloc(1)!
